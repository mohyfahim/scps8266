...
#define ESPNOW_ADDR_IS_EMPTY(addr)                                             \
  (((addr)[0] | (addr)[1] | (addr)[2] | (addr)[3] | (addr)[4] | (addr)[5]) ==  \
   0x0)
#define ESPNOW_ADDR_IS_BROADCAST(addr)                                         \
  (((addr)[0] & (addr)[1] & (addr)[2] & (addr)[3] & (addr)[4] & (addr)[5]) ==  \
   0xFF)
...
#define ESPNOW_RETRANSMIT_MAX_COUNT                                            \
  0x1f                                /**< Maximum number of retransmissions */
...
#define ESPNOW_INIT_CONFIG_DEFAULT()                                           \
  {                                                                            \
    .pmk = {'E', 'S', 'P', '_', 'N', 'O', 'W', '\0'}, .forward_enable = true,  \
    .send_retry_num = 10, .send_max_timeout = pdMS_TO_TICKS(3000),             \
    .qsize = {                                                                 \
        .ack = 8,                                                              \
        .forward = 8,                                                          \
        .group = 8,                                                            \
...
typedef enum {
  ...
  ESPNOW_TYPE_SCPS_NW,
  ESPNOW_TYPE_SCPS_SEC,
  ESPNOW_TYPE_SCPS_DATA,
  ESPNOW_TYPE_SCPS_CONFIG,
 ...
 */
typedef struct {
  uint16_t magic; /**< Unique identifier of each packet. Packets with the same
                     identifier will be filtered. 0: a random number */
  uint8_t channel : 4; /**< Set the channel where the packet is sent,
                          ESPNOW_CHANNEL_CURRENT or ESPNOW_CHANNEL_ALL */
  bool filter_adjacent_channel : 1; /**< Because espnow is sent through HT20, it
                                       can receive packets from adjacent
                                       channels */
  bool filter_weak_signal : 1; /**< When the signal received by the receiving
                                  device is lower than forward_rssi, frame_head
  ...
  bool group : 1;     /**< Only the group set as broadcast transmission mode is
                         valid */
  bool ack : 1;       /**< Wait for the receiving device to return ack to ensure
                         transmission reliability */
  uint16_t retransmit_count : 5; /**< Too many packet retransmissions will lead
                                    to network congestion */
  uint8_t forward_ttl : 5;       /**< Number of hops in data transfer */
  int8_t forward_rssi : 8; /**< When the data packet signal received by the
                              receiving device is lower than forward_rssi, it
                              will not be transferred, in order to avoid network
                              congestion caused by packet transfer */
 ...
 * @param[in]   size  the maximum length of data, must be no more than
 * ESPNOW_DATA_LEN
 * @param[in]   frame_config  if frame_config is NULL, Use
 * ESPNOW_FRAME_CONFIG_DEFAULT configuration
 ...
 */
esp_err_t espnow_send(espnow_type_t type, const espnow_addr_t dest_addr,
                      const void *data, size_t size,
                      const espnow_frame_head_t *frame_config,
 ...
 * @param[in]   group_id  pointer to the specified group ID addresses
 * @param[in]   frame_head  use ESPNOW_FRAME_CONFIG_DEFAULT configuration if
 * frame_config is NULL
...
esp_err_t espnow_send_group(const espnow_addr_t *addrs_list, size_t addrs_num,
                            ...
                            espnow_frame_head_t *frame_head, bool type,
                            TickType_t wait_ticks);
